...
//		System.out.println(m.getMemberId());
	...
	public String MemberSearchList(HttpServletRequest request) {
		...
		MemberPageData mpd = service.MemberSearchList(selectColumn, search, reqPage, selectCount, alignTitle,
				alignStatus);
	...
	public String adminComplainList(Model model, int reqPage, int check, int reqPage2, String search,
			String searchTitle) {
	...
	public String BookSearchRentalList(HttpServletRequest request) {
		...
		BookRentalStatusPage brsp = service.bookSearchRentalStatusList(selectColumn, search, aReqPage, selectCount,
				alignTitle, alignStatus, returnStatus);
		...
		BookRentalStatusSearchPage brssp = new BookRentalStatusSearchPage(list, pageNavi, arrRentStartDate,
				arrRentEndDate, aReqPage, selectCount, compareStatus);
	...
	public String BookSearchRentalApplyList(HttpServletRequest request) {
		...
		System.out.println("선택한 배열 제목 : " + alignTitle);
		System.out.println("선택한 배열 상태 : " + alignStatus);
		BookRentalApplyPage brap = service.bookSearchRentalApplyList(selectColumn, search, aReqPage, selectCount,
				alignTitle, alignStatus);
		...
		System.out.println("pageNavi : " + pageNavi);
		BookRentalApplySearchPage brasp = new BookRentalApplySearchPage(list, pageNavi, arrRentApplyDate, aReqPage,
				selectCount);
	...
	public String BookSearchTurnApplyList(HttpServletRequest request) {
		...
		System.out.println("선택한 배열 제목 : " + alignTitle);
		System.out.println("선택한 배열 상태 : " + alignStatus);
		BookTurnApplyPage btap = service.bookSearchTurnApplyList(selectColumn, search, aReqPage, selectCount,
				alignTitle, alignStatus);
		...
		System.out.println("pageNavi : " + pageNavi);
		BookTurnApplySearchPage brasp = new BookTurnApplySearchPage(list, pageNavi, arrTurnApplyDate, aReqPage,
				selectCount);
		return new Gson().toJson(brasp);
	...
	public String agreeTurnApply(HttpServletRequest request, Model model) {
		...
		ArrayList<Reservation> reservationList = (ArrayList<Reservation>) service.selectReservationListTurnApply(
				bookList.getBookName(), bookList.getBookPublisher(), bookList.getBookWriter());
		...
		if (memberEmail.length != 0) {
			...
			String textContent = "<div style='border:1px solid gray;width:500px;height:480px;text-align:center;margin:0 auto;'>"
					...
					+ "                <td style='font-size:20px; font-weight:bolder;'>" + bookList.getBookName()
					+ "</td>" + "            </tr>" + "            <tr>"
					+ "                <td style='font-size:14px;'>" + Writer + "</td>" + "            </tr>"
			...
			if (result > 0) {
				int reservationResult = service.deleteReservationTurnApply(bookList.getBookName(),
						bookList.getBookPublisher(), bookList.getBookWriter());
	...
	public String BookSearchReservationList(HttpServletRequest request) {
		...
		System.out.println("선택한 배열 제목 : " + alignTitle);
		System.out.println("선택한 배열 상태 : " + alignStatus);
		ReservationPage rp = service.bookSearchReservationList(selectColumn, search, aReqPage, selectCount, alignTitle,
				alignStatus);
	...
	@ResponseBody
	@RequestMapping(value = "/selectDeleteMember.do", produces = "application/json;charset=utf-8")
	public String selectDeleteMember(HttpServletRequest request) {
		String[] checkArr = request.getParameterValues("checkArr");
		System.out.println("배열 사이즈" + checkArr.length);
		int count = 0;
		for (String memberId : checkArr) {
			int result = service.adminDeleteMember(memberId);
			if (result > 0) {
				count++;
			} else {
	...
	@ResponseBody
	@RequestMapping(value = "/selectAgreeTurnApply.do", produces = "application/json;charset=utf-8")
	public String selectAgreeTurnApply(HttpServletRequest request) {
		String[] checkArr = request.getParameterValues("checkArr");
		System.out.println("배열 사이즈" + checkArr.length);
		int count = 0;
		for (String strTurnApply : checkArr) {
			int turnApply = Integer.parseInt(strTurnApply);
			// 반납신청테이블에서 반납신청번호로 정보조회해오기
			TurnApply selectTurnApplyOneList = service.selectTurnApplyOneList(turnApply);
			// 반납신청테이블에서 불러온 목록에 책넘버로 도서테이블에서 책목록 가져오기
			Book bookList = service.selectBookListTurnApply(selectTurnApplyOneList.getBookNo());
			...
			if (memberEmail.length != 0) {
				...
				if (result > 0) {
					...
					if (reservationResult > 0) {
						...
						System.out.println("예약목록 삭제 실패");
					}
				} else {
					System.out.println("반납신청 테이블 삭제 실패");
				}
			} else {
				int result = service.deleteTurnApply(turnApply);
				if (result > 0) {
	...
	@ResponseBody
	@RequestMapping(value = "/selectAgreeRentApply.do", produces = "application/json;charset=utf-8")
	public String selectAgreeRentApply(HttpServletRequest request) {
		String[] checkArr = request.getParameterValues("checkArr");
		System.out.println("배열 사이즈" + checkArr.length);
		int count = 0;
		for (String strRentApply : checkArr) {
			int turnApply = Integer.parseInt(strRentApply);
			RentApply selectRentApply = service.selectOneRentApply(turnApply);
			int insertResult = service.insertAgreeRentApply(selectRentApply);
			if (insertResult > 0) {
				int deleteResult = service.deleteAgreeRentApply(turnApply);
				if (deleteResult > 0) {
					System.out.println("insert,delete성공");
					int updateResult = service.updateBookRentApply(selectRentApply.getBookNo());
					if (updateResult > 0) {
		...
		@ResponseBody
		@RequestMapping(value = "/selectDeleteSpot.do", produces = "application/json;charset=utf-8")
		public String selectDeleteSpot(HttpServletRequest request) {
			String[] checkArr = request.getParameterValues("checkArr");
			System.out.println("배열 사이즈" + checkArr.length);
			int count = 0;
			for (String strSpotNo : checkArr) {
				int spotNo = Integer.parseInt(strSpotNo);
				int result = service.deleteSpot(spotNo);
				System.out.println(result);
				if (result > 0) {
					count++;
